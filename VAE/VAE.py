...
class ConvEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        mu = self.conv_mu(x)         # -> (latent_channels,40,40)
        logvar = self.conv_logvar(x) # -> (latent_channels,40,40)
        return mu, logvar
...
class ConvDecoder(nn.Module):
    ...
    def forward(self, z):
        x = F.relu(self.bn1(self.deconv1(z)))   # 40->40
        x = F.relu(self.bn2(self.deconv2(x)))   # 40->80

        x = F.relu(self.bn3(self.deconv3(x)))   # 80->80
        x = F.relu(self.bn4(self.deconv4(x)))   # 80->160

        x = torch.sigmoid(self.conv_final(x))   # 160->160
        return x
class VAE(nn.Module):
    def __init__(self, config):
